...
function applyPolarPlanetEffect(sourceImage, size, state){
  ...
  // Soft circular edge just outside the unit disk
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      // Atmosphere: cool haze toward the outer rim
      if (atmoAmt > 0.01) {
        ...
        // Slight lift so outer sky feels airy
        const lift = haze * 18;
      ...
      // Vignette: darken toward outer edge (and very lightly at extreme rim)
      if (vignetteAmt > 0.01) {
