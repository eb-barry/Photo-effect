...
export function renderHomeScreen(root, navigate){
  ...
  root.innerHTML = `
    <main class="app-shell">
      <header class="home-header">
        <h1>Photo Effects</h1>
        <p>影像創作工具箱</p>
        ${iconButton({ icon: "settings", label: "設定", id: "settingsBtn", className: "home-settings" })}
      </header>

      <section class="icon-grid" aria-label="功能列表">
        ${features.map(feature => `
          <button
            class="feature-icon ${feature.enabled ? "is-enabled" : "is-disabled"}"
  ...
  root.querySelector("#settingsBtn")?.addEventListener("click", event => {
    event.preventDefault();
    navigate("settings");
  });

  root.querySelectorAll(".feature-icon.is-enabled").forEach(button => {
